...
export class ScatterPlotBase extends React.Component<
  IScatterPlotProps,
  {
    _width: number;
    _height: number;
...
> {
  private _points: IScatterPlotPoints[];
  private _classNames: IProcessedStyleSet<IScatterPlotStyles>;
  ...
  public render(): JSX.Element {
    ...
    if (this._points && this._points.length > 0) {
      this._points.map((chartData: IScatterPlotPoints) => {
    ...
    if (dataPresent) {
      dataType
        ? this._createDateXAxis(tickValues, tickFormat)
        : this._createNumericXAxis();
        ...
        {this.state.isCalloutVisible ? (
          ...
          >
            <div className={this._classNames.calloutContentRoot}>
              <span className={this._classNames.calloutContentX}>
                {this.state.hoverXValue}
              </span>
              <span className={this._classNames.calloutContentY}>
                {this.state.hoverYValue}
              </span>
            </div>
          </Callout>
        ) : null}
  ...
  private _fitParentContainer(): void {
    ...
    if (this.props.parentRef) {
      this._reqID = requestAnimationFrame(() => {
        const currentContainerWidth = this.props.parentRef!.getBoundingClientRect()
          .width;
        const currentContainerHeight =
          this.props.parentRef!.getBoundingClientRect().height > 26
            ? this.props.parentRef!.getBoundingClientRect().height
            : 350;
        const shouldResize =
      ...
      this._reqID = requestAnimationFrame(() => {
        const currentContainerWidth = this.chartContainer.getBoundingClientRect()
          .width;
        const currentContainerHeight =
          this.chartContainer.getBoundingClientRect().height > 26
  ...
  private _createLegends(data: ILineChartPoints[]): JSX.Element {
    const legendDataItems = data.map(
      (point: ILineChartPoints, index: number) => {
        ...
        const legend: ILegend = {
          ...
          action: () => {
            if (
              this.state.activeLegend !== point.legend ||
  ...
  private _createNumericXAxis(): void {
    const xMax = d3Max(this._points, (point: IScatterPlotPoints) => {
      return d3Max(point.data, (item: IScatterPlotDataPoint) => {
    ...
    const xAxisScale = d3ScaleLinear()
      ...
      .range([
        this.margins.left,
        this.state.containerWidth - this.margins.right
      ]);
  ...
  private _prepareDatapoints(
    ...
    const dataPointsArray: number[] =
      minVal > 100 ? [100, val] : includeZero ? [0, val] : [val];
  ...
  private _createDateXAxis = (
    ...
    this._points.map((singleLineChartData: IScatterPlotPoints) => {
      singleLineChartData.data.map((point: IScatterPlotDataPoint) => {
  ...
  private _createYAxis = () => {
    ...
    const yAxis = d3AxisLeft(yAxisScale)
      .tickSize(
        -(this.state.containerWidth - this.margins.left - this.margins.right)
      )
      .tickPadding(12)
